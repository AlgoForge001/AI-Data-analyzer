...
# ─────────────────────────────────────────────
# CONFIG NORMALIZER  (business rules, immutable output)
# ─────────────────────────────────────────────
...
class ChartConfigNormalizer:
    ...
    def normalize(self, schema: ChartConfigSchema) -> dict | None:
        ...
        # ── chart without y: build synthetic count Series, never touch self._df ──
        synthetic: pd.Series | None = None
        synthetic_name: str | None  = None

        if chart_type != "histogram" and y is None:
            ...
            y   = synthetic_name
            agg = "count"
            logger.debug("%s chart: synthetic count column %r for x=%r (no mutation)", chart_type, y, x)
        # ──────────────────────────────────────────────────────────────────────
        ...
        if chart_type == "scatter" and (self._card(x) < 5 or (y and self._card(y) < 5)):
            return None
        if chart_type in ("bar", "pie", "line") and y and y not in effective_nums and agg != "count":
            return None
